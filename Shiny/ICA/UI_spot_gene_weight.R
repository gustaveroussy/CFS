...
output[["spot_gene_heatmap"]] <- plotly::renderPlotly({
  ...
  p <- pheatmap(data@ica[[IC_C]]$spot_top_genes_weight,clustering_method = "ward.D",clustering_distance_cols = "correlation")
  
  col_order <- p[["tree_col"]][["order"]]
  row_order <- p[["tree_row"]][["order"]]
  data@ica[[IC_C]]$spot_top_genes_weight <- data@ica[[IC_C]]$spot_top_genes_weight[,col_order]
  data@ica[[IC_C]]$spot_top_genes_weight <- data@ica[[IC_C]]$spot_top_genes_weight[row_order,]
  
  plot_ly(
    x = colnames(data@ica[[IC_C]]$spot_top_genes_weight), y = rownames(data@ica[[IC_C]]$spot_top_genes_weight),
    z = data@ica[[IC_C]]$spot_top_genes_weight, type = "heatmap", zmin = input$slider_spot_gene_heatmap_range[1], zmax = input$slider_spot_gene_heatmap_range[2],
