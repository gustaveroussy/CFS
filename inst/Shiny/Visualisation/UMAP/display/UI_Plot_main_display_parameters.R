    ...
    if(input$Plot_display_type == "gene"){
      tagList(
        sliderInput("slider_visual_spatial_range", label = "Color range",
                    min = round(min(GetAssay(values$data, assay = values$data@active.assay)[input$gene_UMAP_choice,]), digits = 2), 
                    max = round(max(GetAssay(values$data, assay = values$data@active.assay)[input$gene_UMAP_choice,]), digits = 2),
                    value = c(round(min(GetAssay(values$data, assay = values$data@active.assay)[input$gene_UMAP_choice,]),digits = 2),
                              round(max(GetAssay(values$data, assay = values$data@active.assay)[input$gene_UMAP_choice,]), digits = 2)),
                    step = 0.01),
