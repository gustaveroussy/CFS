...
current_plot_umap <- reactive({
  ...
  if(input$image_display_UMAP){
    TissueCoordinates = TissueCoordinates()[input$Plot_image_spatial]
    ...
    annotation = values$data@misc$annotation
    cell.embeddings.umap = values$data@reductions[[input$Visualisation_selected_dimred_to_display]]@cell.embeddings
  }
  ...
  if(input$interactive_display_visualisation_UMAP | ncol(cell.embeddings.umap) == 3){
    ...
    if (input$Plot_analysis_display_type == "Dimentional reduction"){
      if (input$Plot_display_type == "metadata"){
        if(input$what_to_display_UMAP_choice == "seurat_clusters"){
          ...
          if (ncol(cell.embeddings.umap) == 2){
            fig <- fig %>%
              add_trace(
                ...
                marker = list(
                  color = as.double(meta.data[[input$what_to_display_UMAP_choice]]),
                ...
                showlegend = T,
                text = as.character(meta.data[[input$what_to_display_UMAP_choice]]),
          ...
          if (ncol(cell.embeddings.umap) == 2){
            for (i in unique(meta.data[[input$what_to_display_UMAP_choice]])[order(unique(meta.data[[input$what_to_display_UMAP_choice]]))]){
              fig <- fig %>%
                add_trace(
                  x = cell.embeddings.umap[which(meta.data[[input$what_to_display_UMAP_choice]]==i),1],
                  y = cell.embeddings.umap[which(meta.data[[input$what_to_display_UMAP_choice]]==i),2],
                  name = i,
                  marker = list(
                    color = palette[c],
                    size = input$Plot_scatter_size_UMAP
            ...
            for (i in unique(meta.data[[input$what_to_display_UMAP_choice]])[order(unique(meta.data[[input$what_to_display_UMAP_choice]]))]){
              fig <- fig %>%
                add_trace(
                  x = cell.embeddings.umap[which(meta.data[[input$what_to_display_UMAP_choice]]==i),1],
                  y = cell.embeddings.umap[which(meta.data[[input$what_to_display_UMAP_choice]]==i),2],
                  z = cell.embeddings.umap[which(meta.data[[input$what_to_display_UMAP_choice]]==i),3],
                  name = i,
                  marker = list(
        ...
        cell.embeddings <- values$data@reductions[[input$Plot_display_type]]@cell.embeddings[(rownames(values$data@reductions[[input$Plot_display_type]]@cell.embeddings) %in% unlist(c(sapply(TissueCoordinates,rownames)))),]
    ...
    if (input$Plot_analysis_display_type == "Dimentional reduction"){
      if (input$Plot_display_type == "metadata"){
        ...
        fig = ggplot() +
          geom_point(aes(cell.embeddings.umap[,1], cell.embeddings.umap[,2], color=scale.data[input$what_to_display_UMAP_choice,]), size = input$Plot_scatter_size_UMAP) +
          ggplot2::scale_color_gradientn(name = input$what_to_display_UMAP_choice,
                                         colours = viridis_pal(option = if(input$select_color_visualisation_projection %in% c("A","B","C","D","E","F","G","H")){input$select_color_visualisation_projection}else{"D"})(ncol(values$data)), limits=c(input$slider_visual_spatial_range[1], input$slider_visual_spatial_range[2]), oob=squish) +
          guides(size = "none") +
          theme_void()
        ...
        if(limits[1] == limits[2]){
          limits = c(min(cell.embeddings[,dimension_query]),max(cell.embeddings[,dimension_query]))
        }
        
        fig = ggplot() +
          geom_point(aes(cell.embeddings.umap[,1], cell.embeddings.umap[,2], color=cell.embeddings[,dimension_query]), size = input$Plot_scatter_size_UMAP) +
          ggplot2::scale_color_gradientn(name = input$what_to_display_UMAP_choice,
