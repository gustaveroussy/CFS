  ...
  if(input$interactive_display_visualisation_spatial){
    ...
    if (input$Plot_analysis_display_type == "Dimentional reduction"){
      if (input$Plot_display_type == "seurat_clusters"){
        ...
        coordinates = lapply(input$Plot_image_spatial, function(sample){coordinates[[sample]] = cbind(coordinates[[sample]],values$data@assays$SCT@data[input$gene_UMAP_choice, rownames(coordinates[[sample]])]); colnames(coordinates[[sample]]) = c("imagerow","imagecol","value");return(coordinates[[sample]])})
        names(coordinates) = input$Plot_image_spatial
        ...
        coordinates = lapply(input$Plot_image_spatial, function(sample){coordinates[[sample]] = cbind(coordinates[[sample]],data@reductions$ica@cell.embeddings[rownames(coordinates[[sample]]), input$IC_UMAP_choice]); colnames(coordinates[[sample]]) = c("imagerow","imagecol","value");return(coordinates[[sample]])})
        names(coordinates) = input$Plot_image_spatial
