    ...
    if(input$Plot_cluster_reduction_to_use == "ica"){
      if (!is.null(input$Plot_cluster_IC_choice)) {
        if (!is.null(input$Plot_cluster_type_UMAP_choice)){
          ...
          values$data = FindNeighbors(values$data, reduction = input$Plot_cluster_reduction_to_use,dims = as.integer(gsub('[IC_]','',unique(c(type,input$Plot_cluster_IC_choice)))))
        ...
        incProgress(0.4, detail = "Finding neighbors")
        values$data = FindNeighbors(values$data, reduction = input$Plot_cluster_reduction_to_use,dims = as.integer(gsub('[IC_]','',type)))
        incProgress(0.4, detail = "Finding clusters")
        values$data = FindClusters(values$data, verbose = FALSE, resolution=input$Clustering_resolution, method = "igraph", algorithm = as.integer(input$select_algorithm_clusterisation), cluster.name = input$cluster_named_by_user)
        incProgress(0.1, detail = "Done")
      ...
      values_test = which(values$data@misc$reduction_names[[input$Plot_cluster_reduction_to_use]] %in% input$Plot_cluster_IC_choice)
      
      incProgress(0.4, detail = "Finding neighbors")
      values$data = FindNeighbors(values$data, reduction = input$Plot_cluster_reduction_to_use, dims = values_test)
      incProgress(0.4, detail = "Finding clusters")
      values$data = FindClusters(values$data, verbose = FALSE, resolution=input$Clustering_resolution, method = "igraph", algorithm = as.integer(input$select_algorithm_clusterisation), cluster.name = input$cluster_named_by_user)
      incProgress(0.1, detail = "Done")
      values$data@meta.data[,input$cluster_named_by_user] = as.factor(values$data@meta.data[,input$cluster_named_by_user])
