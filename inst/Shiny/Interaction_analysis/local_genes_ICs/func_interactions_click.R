...
clicked_interaction_IC_genes_plot <- reactive({
  req(values$HD_image_2)
  if(!is.null(current_plot_graph_interactions_IC_genes())){
    return(plotly::event_data(c("plotly_click"), source = "U"))
  }
})

observeEvent(clicked_interaction_IC_genes_plot(), {
  req(values$HD_image_2)
  table = clicked_interaction_IC_genes_plot()
